...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--require-results",action="store_true"); args=parser.parse_args(); required=["README.md","RUNBOOK.md","data/locomo_selection.json","data/lifecycle_cases.json","project8_long_term_memory.ipynb","src/project8_agent/memory.py","src/project8_agent/local_models.py","tools/fetch_locomo.py","tools/generate_report.py","background/project8_background.md"]; errors=[f"missing {p}" for p in required if not (ROOT/p).exists()]
    ...
    for p in ROOT.rglob("*"):
        ...
        if ROOT/"data/cache/locomo" in p.parents: continue
