...
def evaluate_project7(cases:list[GatewayCase],output_dir:str|Path,reviewer:object|None=None)->dict[str,Any]:
    ...
    review_summary={"mode":"not_run","cases":0,"accuracy_pct":None,"model_calls":0}
    if review_rows:
        review_summary={"mode":"openai","cases":len(review_rows),"accuracy_pct":_pct(sum(r["recommendation_correct"] for r in review_rows),len(review_rows)),"model_calls":getattr(reviewer,"calls",len(review_rows))}
